...
当前项目的“活动描述”已经调整为 **Quill CDN + JSX 页面内嵌编辑器** 方案，不再依赖 CloudBase 设计器里额外挂载 `WdRichText / RichTextView` 插槽组件。

## 当前方案

- 编辑端使用仓库内置 `RichTextEditor`，其内部通过 **Quill CDN** 动态加载成熟富文本编辑器
...
因此当前改为使用 **Quill CDN 富文本编辑器**，先保证“后台编辑 -> 保存 -> 后台查看 -> 小程序展示”链路稳定可用。

## 当前源码中的闭环

### 1. 编辑

活动管理弹窗中的“活动描述”字段直接渲染 `RichTextEditor`，内部由 Quill 接管编辑体验，支持：
...
- 如果 HTML 中包含 `cloud:// / tcb://` 图片，会自动换成临时访问链接后再展示
...
1. Quill 编辑器通过 CDN 动态加载，不依赖 npm `node_modules`
2. 编辑器和查看组件显式声明 `ol / ul` 的列表样式
3. 富文本规范化结果中保留 `list-style-type`
4. 后台查看态和小程序展示态都支持 `cloud:// / tcb://` 图片临时链接转换
5. 纯文本回退时保留换行转 `<br />`
